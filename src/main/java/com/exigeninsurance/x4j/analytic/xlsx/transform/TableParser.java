...
public final class TableParser {
	...
	public void parse(XSSFTable table) {
		...
		for( int i = 0; i < template.getLastCellNum(); i++){
			...
			buildTableCell(table,templateNode, template.getCell(i, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK));
	...
	void buildTableCell(XSSFTable table,
	...
	void addTotals(TableNode tableNode, Node templateNode) {
		...
		XSSFTable table = tableNode.getTable();
		int totalsRowCount = (int) table.getCTTable().getTotalsRowCount();
		...
		if(totalsRowCount > 0){
			...
			for( CTTableColumn ctCol  : table.getCTTable().getTableColumns().getTableColumnArray()){
				if(ctCol.isSetTotalsRowFunction()){							
					...
					XSSFCell cell = sheet.getRow(endRow).getCell(index, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK);
	...
	private void insertBlanks(Node templateNode, XSSFRow tRow, Node totalsRow, int lastCol) {
		...
		for (int i = 0; i < lastCol; i++) {
			XSSFCell cell = tRow.getCell(i, Row.MissingCellPolicy.CREATE_NULL_AS_BLANK);
