...
public class PictureParser {
    ...
    private Picture parsePicture() throws Exception {
        PackageRelationship pr = XSSFSheetHelper.getPackageRelationship(pictureData);

        InputStream in = pr.getSource().getInputStream();
        ...
        Node anchor = getAnchorNode(document, pr.getId());
        ...
        Picture picture = new Picture(pictureData, pr.getId(), fromRow, fromCol, toRow, toCol);
    ...
    private Map<String, XSSFPictureData> findLegacyPictureData(POIXMLDocumentPart legacyDrawing) {
        ...
        for (POIXMLDocumentPart part : legacyDrawing.getRelations()) {
            if (part instanceof XSSFPictureData) {
                pictureData.put(XSSFSheetHelper.getPackageRelationship(part).getId(), (XSSFPictureData) part);
            }
        }
        return pictureData;
    }
    ...
    private POIXMLDocumentPart findLegacyDrawingPart(XSSFSheet sheet) {
        ...
        if (legacyDrawingHF != null) {
            ...
            return sheet.getRelationById(id);
        }
        return null;
    }
