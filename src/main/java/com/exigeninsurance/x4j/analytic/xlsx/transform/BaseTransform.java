...
abstract public class BaseTransform implements Transform {
	...
	public final void process(ReportContext reportContext, InputStream in, File saveTo) {
		File template = null;
		XLSXWorkbook workbook = null;
		try {
			template = createWorkbookFile();
			...
			log.error("Exception occurred during report processing", e);
		} finally {
