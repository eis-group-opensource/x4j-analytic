...
final public class XLSXStylesTable extends StylesTable {
	...
	public XLSXStylesTable(PackagePart part) throws IOException {
		super(part);
	}
	...
	private CTXf copyStyle(XLSXStylesTable styles, CTCellStyle item) {
		...
		XSSFCellFill fill = new XSSFCellFill( (CTFill) styles.getFillAt(fillIdx).getCTFill().copy(), null);
		fillIdx = putFill(fill);
		cellXf.setFillId(fillIdx);
		
		short formatIdx = (short) cellXf.getNumFmtId();
		
		if(formatIdx > 0){
			String format = styles.getNumberFormatAt(formatIdx);
			formatIdx = (short) putNumberFormat(format);
