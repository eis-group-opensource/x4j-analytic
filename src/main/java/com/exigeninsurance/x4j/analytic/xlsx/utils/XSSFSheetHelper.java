...
public final class XSSFSheetHelper {
    ...
    public static PackageRelationship getPackageRelationship(POIXMLDocumentPart part) {
        try{
            String partName = part.getPackagePart().getPartName().getName();
            for (PackageRelationship pr : part.getParent().getPackagePart().getRelationships()) {
    ...
    private static boolean rowIsInsideDefinedArea(int rowNum, CTDefinedName printArea) {
        ...
        AreaReference areaRef = new AreaReference(printArea.getStringValue(), null);
