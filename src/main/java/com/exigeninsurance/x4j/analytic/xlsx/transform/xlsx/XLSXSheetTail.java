...
public class XLSXSheetTail extends Node {
    ...
    private String tablePart(POIXMLDocumentPart part) {
        return "<tablePart r:id=\"" + XSSFSheetHelper.getPackageRelationship(part).getId() + "\" />";
    }

    private boolean isTablePart(POIXMLDocumentPart next) {
        return XSSFRelation.TABLE.getRelation().equals(XSSFSheetHelper.getPackageRelationship(next).getRelationshipType());
    }
